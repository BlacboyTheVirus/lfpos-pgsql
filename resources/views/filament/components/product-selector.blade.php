    ...
    <div class="flex flex-wrap gap-3">
        @foreach ($products as $product)
            <button
                type="button"
                x-data
                x-on:click="$wire.addProductToForm({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price }}, {{ $product->default_width ?? 1.0 }}, {{ $product->minimum_amount ?? 0 }})"
                wire:loading.attr="disabled"
                wire:target="addProductToForm({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price }}, {{ $product->default_width ?? 1.0 }}, {{ $product->minimum_amount ?? 0 }})"
